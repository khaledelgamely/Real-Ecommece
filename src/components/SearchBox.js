...
const SearchBox = () => {
  ...
  return (
    <div className='searchBox-Component'>
      ...
      <div className=' filterRevHold '>
        <p className='frText fs-1'> Filter Reviews </p>
        <input type="text" placeholder="Search" className='searchBox ' onChange={e => setSearch(e.target.value)} />
      </div>
      
      <p className='popularKeyWordsText fs-4'> POPULAR KEYWORDS </p>

      <div className='keyWordHold my-4 '>
        {[...new Set(names.map(item => item.type))].map((item) =>(
          
          <div className='keyWord rounded-2' onClick={()=>getHandler(item)}>
            <p >{item.trim()}</p>
          </div>
        ))}
      ...
      <div className='searchItemsHold'>
        {names.filter(i =>{
        ...
        else if (i.description.toLowerCase().includes(search.toLowerCase())) {
          return i;
        }}).slice(0, 6).map(item => (
          <div className='searchList'>
            <p className='fw-bold'>{item.name}</p>
            <p className='my-1'>  {item.stars === 4 ? <div className='d-flex'> <AiFillStar /> <AiFillStar /> <AiFillStar /> <AiFillStar />  </div> : <div className='d-flex'> <AiFillStar /> <AiFillStar /> <AiFillStar /> <AiFillStar /> <AiFillStar />  </div>} </p>
            <p className='searchList-heading'>{item.heading}</p>
            <Highlighter
            className='mobdesc'
          ...
          <p className='firstLetterHold'> {item.name.charAt(0)} </p>
          </div>
        ))
